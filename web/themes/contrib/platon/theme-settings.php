...
function platon_form_system_theme_settings_alter(&$form, FormStateInterface &$form_state, $form_id = NULL) {
  ...
  if (\Drupal::moduleHandler()->moduleExists('opigno_mobile_app')) {
    ...
    // Functionality to view this logo is available only in mobile app not in Opigno.
    $form['platon_mobile_app'] = [
      '#type' => 'details',
    ...
    $form['platon_mobile_app']['mobile_app_premium_logo'] = [
      '#type' => 'managed_file',
      '#title' => t('Premium logo'),
  ...
  $form['browser_sync'] = [
    ...
    '#default_value' => theme_get_setting('browser_sync'),
  ];
