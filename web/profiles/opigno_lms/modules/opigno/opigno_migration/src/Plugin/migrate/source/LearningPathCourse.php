 ...
 * )
 */
class LearningPathCourse extends FieldableEntity {
  /**
   * The module handler.
   *
  ...
  public function prepareRow(Row $row) {
    ...
    // Set body of learning path.
    $lp_body = $connection->select('field_revision_body', 'f_r_b')
    ...
    $row->setSourceProperty('body', $lp_body);

    // Set certificate for learning path.
    $lp_certificate = $connection->select('certificate_node', 'c_n')
      ->fields('c_n', ['template'])
      ->condition('nid', $nid)
      ->execute()
      ->fetchField();
    ...
    if ($lp_certificate > 0) {
      $row->setSourceProperty('field_certificate', $lp_certificate);
    }

    // Set visibility for learning path.
    $lp_visibility = $connection->select('field_data_group_access', 'f_d_g_a')
      ->fields('f_d_g_a', ['group_access_value'])
      ->condition('entity_id', $nid)
      ->execute()
      ->fetchField();

    switch ($lp_visibility) {
      case 0:
        $visibility = 'public';
        break;
    ...
    if (!empty($visibility)) {
      $row->setSourceProperty('field_learning_path_visibility', $visibility);
    }

    // Set requires validation for learning path.
    $lp_validation = $connection->select('field_data_requires_validation', 'f_d_r_v')
      ->fields('f_d_r_v', ['requires_validation_value'])
      ->condition('entity_id', $nid)
      ->execute()
      ->fetchField();

    $row->setSourceProperty('field_requires_validation', $lp_validation);
