    ...
    foreach ($content_types_definitions as $content_type_definition) {
      ...
      if (in_array($group_type->id(), $content_type_definition['allowed_group_types'])) {
        ...
        $available_types[] = [
          ...
          'name' => $this->t($content_type_definition['readable_name']),
        ];
      }
    }
...
//      }
  ...
  public function trainingExport(Group $group) {
    ...
    foreach ($group_content as $entity) {
      ...
      elseif(in_array($entity->bundle(), ['opigno_moxtra_meeting', 'opigno_ilt'])) {
    ...
    if (!empty($data_structure[$group->id()]['field_learning_path_media_image'][0]['target_id'])) {
      ...
      $media = Media::load($data_structure[$group->id()]['field_learning_path_media_image'][0]['target_id']);
      if (!empty($media)) {
  ...
  protected function getMaxScore($module, $activity) {
    $query = \Drupal::service('database')->select('opigno_module_relationship', 'omr')
      ->fields('omr', [])
