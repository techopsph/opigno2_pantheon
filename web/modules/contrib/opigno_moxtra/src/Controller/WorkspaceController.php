...
class WorkspaceController extends ControllerBase {
  ...
  public function workspaceList($current_workspace_id = NULL, $learning_path_iframe = NULL) {
    ...
    // Add access to workspace for user if needed.
    if (empty($workspace_ids)) {
      $group = Group::load($group_id);
      $user = User::load($user->id());

      if ($group->getMember($user) && $user->hasRole('collaborative_features')) {
        $current_workspace_entity = Workspace::load($current_workspace_id);
        $current_workspace_entity->addMember($user->id());
        $current_workspace_entity->save();
        $workspace_ids[] = $current_workspace_id;
      }
    }
    ...
    $items = array_map(function ($workspace) use ($learning_path_iframe) {
      if ($workspace->access('edit')) {
        ...
        $workspace_link_rendered = \Drupal::service('renderer')->render($workspace_link);
        ...
        $workspace_edit_rendered = \Drupal::service('renderer')->render($workspace_edit);
