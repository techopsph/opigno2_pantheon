...
class TincanContentAnswerAssistant {
  ...
  protected function getScoreFromLrs($activity_id, $registration, $max_score) {
    ...
    if (!$lrs) {
      $messenger->addWarning(t('Connection to the LRS failed'));
      return 0;
    }

    // If we have the connection, get the statement.
    if (empty($registration)) {
      $messenger->addWarning(t('There was an error while answering the question, please go back and try again.'));
      return 0;
    }

    if (empty($activity_id)) {
      $messenger->addWarning(t('Error while obtaining the activity ID. Maybe a malformed TinCan package.'));
      return 0;
    }

    $score_statement = $this->getStatementFinalScore($lrs, $registration, $activity_id);
    if (!$score_statement) {
      $messenger->addWarning(t('Can not get score from LRS. Check your LRS settings.'));
      return 0;
    }
  ...
  protected function getLrsConnection() {
    ...
    if (empty($endpoint) || empty($username) || empty($password)) {
      $messenger->addWarning('Please configure first the Opigno TinCan API module. Go to @url',
        [
