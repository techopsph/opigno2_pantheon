...
class ImportTrainingForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $file = file_save_upload('training_opi', $validators, $temporary_file_path);
    ...
    if (in_array('list_of_files.json', $files)) {
      ...
      $file_path = $temporary_file_path . '/' . $content['training'];
      $real_path = \Drupal::service('file_system')->realpath($file_path);
      $file = file_get_contents($real_path);
      $training_content_array = $serializer->decode($file, $format);
      $training_content = reset($training_content_array);

      $new_training = Group::create([
        'type' => 'learning_path',
        'langcode' => $training_content['langcode'][0]['value'],
        'label' => $training_content['label'][0]['value'],
        'field_guided_navigation' => $training_content['field_guided_navigation'][0]['value'],
        'field_learning_path_enable_forum' => $training_content['field_learning_path_enable_forum'][0]['value'],
        'field_learning_path_published' => $training_content['field_learning_path_published'][0]['value'],
        'field_learning_path_visibility' => $training_content['field_learning_path_visibility'][0]['value'],
      ]);

      if (!empty($training_content['field_learning_path_description'][0])) {
        $new_training->field_learning_path_description->value = $training_content['field_learning_path_description'][0]['value'];
      ...
      foreach ($content['courses'] as $course_name => $course_path) {
        $file_path = $temporary_file_path . '/' . $course_path;
        $real_path = \Drupal::service('file_system')->realpath($file_path);
        $file = file_get_contents($real_path);
      ...
      foreach ($content['terms'] as $term_file_name) {
        $file_path = $temporary_file_path . '/library/' . $term_file_name;
        $real_path = \Drupal::service('file_system')->realpath($file_path);
        $file = file_get_contents($real_path);
      ...
      foreach ($content['files'] as $document) {
        $file_path = $temporary_file_path . '/library/' . $document['file'];
        $real_path = \Drupal::service('file_system')->realpath($file_path);
        $file = file_get_contents($real_path);
        ...
        $current_timestamp = \Drupal::time()->getCurrentTime();
        $date = date('Y-m', $current_timestamp);
        $file_source = $temporary_file_path . '/library/' . $file_content['fid'][0]['value'] . '-' . $file_content['filename'][0]['value'];
        $dest_folder = 'public://' . $date;
        $destination = $dest_folder . '/' . $file_content['filename'][0]['value'];
        \Drupal::service('file_system')->prepareDirectory($dest_folder, FileSystemInterface::MODIFY_PERMISSIONS | FileSystemInterface::CREATE_DIRECTORY);
        $uri = \Drupal::service('file_system')->copy($file_source, $destination, FileSystemInterface::EXISTS_RENAME);
        ...
        $file_path = $temporary_file_path . '/library/' . $document['media'];
        $real_path = \Drupal::service('file_system')->realpath($file_path);
        $file = file_get_contents($real_path);
        $file_content = $serializer->decode($file, $format);
