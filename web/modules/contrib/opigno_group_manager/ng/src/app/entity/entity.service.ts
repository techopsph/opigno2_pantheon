...
export class EntityService {
  ...
  constructor(private http: Http, private appService: AppService) {
    ...
    this.getGuidedNavigationUrl = window['appConfig'].getGuidedNavigationUrl;
  ...
  getGuidedNavigation(mainId): Observable<Entity[]> {
    return this.http
    .get(this.apiBaseUrl + this.appService.replaceUrlParams(this.getGuidedNavigationUrl, { '%groupId': mainId }))
    .map(response => response.json() as Entity[]);
  }
  ...
  traceLinks(entities, links, entityPositions): void {
    entities.forEach((entity) => {
      entity.parents.forEach((_parent) => {
        ...
        link.activities = this.getRequiredActivities(entity, parent);
  ...
  getRequiredActivities(entity, parent): number {
    let activities: any = null;

    entity.parents.forEach((_parent) => {
      if (_parent.cid == parent.cid) {
        activities = _parent.activities;
      }
    });
