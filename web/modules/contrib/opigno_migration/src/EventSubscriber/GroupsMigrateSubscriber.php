  ...
  public function onPostRowSave(MigratePostRowSaveEvent $event) {
    ...
    if ($migration_id == 'opigno_learning_path_course' || $migration_id == 'opigno_learning_path_class') {
      ...
      foreach ($members_ids as $member_id) {
        $account = User::load($member_id->etid);
    ...
    if ($migration_id == 'opigno_activity_h5p' && $completed) {
      ...
      foreach ($modules_statistic as $module_stat) {
        if (in_array($module_stat->entity_id, $modules_ids) && !is_null($lps_results[$module_stat->opigno_statistics_user_course_fk]['uid'])) {
          ...
          $entity = \Drupal::entityTypeManager()->getStorage('user_module_status')->create($values);
          $entity->save();
          $ums_id = $entity->id();
            ...
            foreach ($questions_info as $question) {
              ...
              switch ($question_type) {
                case 'h5p_content':
                  ...
                  $entity = \Drupal::entityTypeManager()->getStorage('opigno_answer')->create($values);
                  $entity->save();
                  break;
