...
class EntityQueryAlter implements ContainerInjectionInterface {
  ...
  protected function doAlter(SelectInterface $query, EntityTypeInterface $entity_type, $operation) {
    ...
    // @todo Add a 'plugin' base field to group content in a next release so we
    // can avoid this lookup and query directly on plugin ID further down.
    ...
    // @todo Could directly query on plugin here if we had that field.
    ...
    if (!empty($allowed_any_ids[CGPII::SCOPE_GROUP_TYPE])) {
      ...
      // precedence.
      if (!empty($all_ids[CGPII::SCOPE_GROUP])) {
        $sub_condition->condition('gcfd.gid', $all_ids[CGPII::SCOPE_GROUP], 'NOT IN');
      }
